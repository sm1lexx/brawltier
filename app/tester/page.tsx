...
export default function TesterPage() {
  ...
  // ✅ Главная функция — сохраняет тир в БД и обновляет топ
  const completeRequest = async (tier: string) => {
    ...
    // 1. Обновляем статус заявки + сохраняем тир
    await supabase
      .from('test_requests')
      .update({
        status: 'completed',
    ...
    // 3. ✅ Сохраняем результат в tier_results
    // Если уже есть результат для этого бойца — обновляем только если новый тир лучше
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="space-y-3">
                  {myTesterRequests.map((req) => (
                    <div key={req.id} className="bg-[#111111] border border-orange-500/20 rounded-2xl p-5 flex items-center gap-4">
                      <img
                        src={req.brawlers?.icon_url || 'https://via.placeholder.com/48'}
                        alt=""
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-400">
                          {req.brawlers?.name} • {req.player_tag} • {timeAgo(req.created_at)}
                        </div>
                      </div>
                      <button
                      ...
                      <button
                        onClick={() => startComplete(req)}
                        className="bg-orange-500 hover:bg-orange-600 px-4 py-2 rounded-xl text-sm font-bold transition"
                      >
                        ✅ Завершить
                      </button>
                    </div>
              ...
              waitingRequests.map((req) => (
                <div key={req.id} className="bg-[#111111] border border-white/10 rounded-2xl p-5 flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="bg-green-500 hover:bg-green-600 px-5 py-2.5 rounded-2xl font-bold text-sm transition"
                  >
                    ✅ Принять
                  </button>
                </div>
        ...
        {activeTab === 'chat' && activeChat && (
          <div
            className="bg-[#111111] border border-white/10 rounded-3xl overflow-hidden flex flex-col"
            style={{ height: '600px' }}
          >
            <div className="px-6 py-4 border-b border-white/10 flex items-center gap-4">
              <img
                src={activeChat.brawlers?.icon_url || 'https://via.placeholder.com/40'}
                alt=""
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">
                  {activeChat.brawlers?.name} • {activeChat.player_tag}
                </div>
              </div>
